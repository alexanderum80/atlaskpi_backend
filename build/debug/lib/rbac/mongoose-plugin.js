...
function rbacPlugin(schema, options) {
    options || (options = {});
    schema.add({
        roles: [{
    ...
    schema.methods.hasRole = function (role, done) {
        var obj = this;
        utils_1.resolveRole(this, role, function (err, role) {
            if (err)
                return done(err);
            var hasRole = false;
        ...
        obj.populate('roles', function (err, obj) {
            ...
            if (obj.roles) {
                ...
                async.until(function () {
                    return hasAny || iter_1 === obj.roles.length;
                }, function (callback) {
                    obj.roles[iter_1].canAny(actionsAndSubjects, function (err, has) {
                        if (err)
                            return callback(err);
                        if (has)
                            hasAny = true;
