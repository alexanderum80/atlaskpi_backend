...
import { EnrollmentNotification } from './notifications/users/index';
...
export class AccountsService {
        ...
        return new Promise < IMutationResponse > ((resolve, reject) => {
                ...
                that._accounts.model.create(input.account, (err, newAccount: IAccountDocument) => {
                        ...
                        createClusterDbUserIfNeeded(that._config.mongoDBAtlasCredentials, newAccount, accountDbUser)
                            .then(success => createUserDatabase(newAccount, firstUserInfo, accountDatabaseName, that._config.newAccountDbUriFormat, that._appConnectionPool, that._enrollmentNotification)
    ...
    function createUserDatabase(newAccount: IAccountDocument, firstUser: ICreateUserDetails,  databaseName: string, newAccountDbUriFormat: string, connectionPool: AppConnectionPool, enrollmentNotification: EnrollmentNotification) {
        ...
        return new Promise < boolean > ((resolve, reject) => {
            ...
            connectionPool.getConnection(databaseObject.uri)
                ...
                .then(() => seedApp(newAccountContext))
                .then(() => generateFirstAccountToken(that, newAccountContext, account, firstUser, ip, clientId, clientDetails))
                .then(token => {
