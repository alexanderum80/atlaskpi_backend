...
import { IChartFormatModel, IChartFormat, IChartFormatDetails, IChartFormatDocument } from './IChartFormat';
import { IMutationResponse, MutationResponse, IPaginationDetails, IQueryResponse } from '../../common';
...
ChartFormatSchema.statics.updateChartFormat = function (id, data) {
    ...
    return new Promise<IMutationResponse>((resolve, reject) => {
        ...
        (<IChartFormatModel>this).findById(id).then((chart) => {
            ...
            let errors = (<any>validate)({ id: id, document: chart }, constraints, { fullMessages: false });
            if (errors) {
                resolve(MutationResponse.fromValidationErrors(errors));
                return;
            }
            ...
            if (data.type) { chart.type = data.type; };
            if (data.typeFormat) {
                 if (data.typeFormat.before) { chart.typeFormat.before = data.typeFormat.before; };
                 if (data.typeFormat.before) { chart.typeFormat.before = data.typeFormat.before; };
                 if (data.typeFormat.after) { chart.typeFormat.after = data.typeFormat.after; };
                 if (data.typeFormat.decimal) { chart.typeFormat.decimal = parseFloat(data.typeFormat.decimal); };
                 if (data.typeFormat.formula) { chart.typeFormat.formula = data.typeFormat.formula; };
            };
...
export function getChartFormatModel(m: mongoose.Connection): IChartFormatModel {
    return <IChartFormatModel>m.model('ChartFormat', ChartFormatSchema, 'chartFormats');
}